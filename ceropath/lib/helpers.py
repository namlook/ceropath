        ...
        if child.tag == '{http://www.w3.org/2000/svg}text':
            ...
            if species.get(individual_id):
                if species[individual_id]['voucher_barcoding']:
                    individual_link = root.makeelement('ns0:a', {'target':'_blank', 'fill': fill_color or '#1139E5', 'xlink:href':"/rdbsea/individual/%s" % individual_id })
                    individual_link.text = individual_id.upper()
                    child.append(individual_link)
                    child.text = ""
                else:
                    child.text = individual_id.upper()
                species_link = root.makeelement('ns0:a', {'target':'_blank', 'fill': fill_color or "#FFB010", 'xlink:href':"/rdbsea/species/%s" % species[individual_id]['species_id']})
